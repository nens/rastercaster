...
class settingsObject:
    ...
    def read(self, inifile):
        ...
        self.elevation_point_search_radius = config.get(
            "parameters", "elevation_point_search_radius"
        )
...
def db_connection(host, port, dbname, user, password):
    db_conn = "dbname={dbname_value} user={user_value} host={host_value} port={port_value} password={password_value}".format(
        dbname_value=dbname,
        user_value=user,
        host_value=host,
        port_value=port,
...
def install(settings):
    ...
    answer = input(
        r"Existing rastercaster data in the database will be overwritten. Are you sure? (Y/N): "
    )
    if answer in ("Y", "y", "J", "j", "prima joh"):
        ...
        execute_sql_command(
            ...
            username=settings.username,
            password=settings.password,
            sql="""CREATE SCHEMA IF NOT EXISTS rc;""",
        )
        ...
        execute_sql_command(
            ...
            username=settings.username,
            password=settings.password,
            sql="""DELETE FROM rc.instellingen; INSERT INTO rc.instellingen VALUES ('pixelsize','{p}'), ('filler_seg_dist','{f}'), ('elevation_point_search_radius', '{e}');""".format(
                p=settings.pixelsize,
                f=settings.filler_seg_dist,
                e=settings.elevation_point_search_radius,
            ),
        )
...
def cast_raster(settings):
    ...
    try:
        sql = (
            """SELECT id FROM rc.surface_admin WHERE exported < updated ORDER BY id;"""
        )
        cur.execute(sql)
        ...
        for id in export_ids:
            try:
                execute_sql_command(
                    ...
                    username=settings.username,
                    password=settings.password,
                    sql="""SELECT rc.update_raster({id});""".format(id=id),
                )
            except:
                has_failed = True
    ...
    for id in all_ids:
        try:
            execute_sql_command(
                ...
                username=settings.username,
                password=settings.password,
                sql="""SELECT rc.update_filler(id) FROM rc.surface WHERE id = {id} AND definition_type = 'filler';""".format(
                    id=id
                ),
            )
        except:
    ...
    if len(all_ids) > len(
        failed_ids
    ):  # prevent Exception when no surface has successfully been cast
        ...
        for id in all_ids:
            if id not in failed_ids:
                ...
                execute_sql_command(
                    ...
                    username=settings.username,
                    password=settings.password,
                    sql="""UPDATE rc.surface_admin SET exported = now() WHERE id = {id};""".format(
                        id=id
                    ),
                )
        ...
        if must_fill_open_pixels:
            ...
            """.format(
                w=cast_inmem_ds.RasterXSize,
                h=cast_inmem_ds.RasterYSize,
                x=cast_inmem_ds.GetGeoTransform()[0],
                y=cast_inmem_ds.GetGeoTransform()[3],
            )
            ...
            mask_in_db_string = "PG:host='{h}' port={p} dbname='{d}' user='{u}' password='{pw}' schema='rc' table='fillnodatamask' mode='2'".format(
                h=settings.host,
                p=settings.port,
                d=settings.database,
                u=settings.username,
            ...
            dsOut = driver.Create(
                str(mask_inmem2), m.RasterXSize, m.RasterYSize, 1, m_band.DataType
            )
        ...
        if settings.base_raster != "":
            ...
            last_db_raster_sr = osr.SpatialReference(
                wkt=last_db_raster_ds.GetProjection()
            )

            if base_raster_sr.GetAttrValue(
                str("authority"), 0
            ) == last_db_raster_sr.GetAttrValue(
                str("authority"), 0
            ) and base_raster_sr.GetAttrValue(
                str("authority"), 1
            ) == last_db_raster_sr.GetAttrValue(
                str("authority"), 1
            ):
                if base_raster_sr != last_db_raster_sr:
                    print(
                        """NOTICE: Base raster and database rasters have different definitions of the same SRS ({a}:{c}). Casting will take some extra time.""".format(
                            a=base_raster_sr.GetAttrValue(str("authority"), 0),
                            c=base_raster_sr.GetAttrValue(str("authority"), 1),
                        )
                    )
                    tr = gdal.Translate(
                        destName="/vsimem/baseraster.tif", srcDS=base_raster_ds
                    )
                    base_raster_inmem = gdal.Open("/vsimem/baseraster.tif", GA_Update)
                    res = base_raster_inmem.SetProjection(
                        last_db_raster_ds.GetProjection()
                    )
                    res = None
        ...
        tmp = gdal.BuildVRT(
            ...
            srcNodata=-9999,
            VRTNodata=-9999,
            outputSRS="EPSG:28992",
        )
        tmp = None

        tmp = gdal.Translate(
            destName=settings.output_file,
            srcDS=vrtname,
            noData=-9999,
            stats=True,
            outputType=gdal.GDT_Float32,
        ...
        if settings.base_raster == "":
            print(
                """WARNING! No surfaces were successfully cast. Returning no result."""
            )
        else:
            print(
                """WARNING! No surfaces were successfully cast. Returning base raster."""
            )
            base_raster_ds = gdal.Open(settings.base_raster)
            tmp = gdal.Translate(
                destName=settings.output_file,
                srcDS=base_raster_ds,
                noData=-9999,
                stats=True,
                outputType=gdal.GDT_Float32,
                creationOptions=["COMPRESS=DEFLATE"],
    ...
    if has_failed == True:
        print(
            "Warning: rasters for some surfaces were not succesfully generated. IDs: {f}".format(
                f=failed_ids
            )
        )
    else:
    ...
    if kwargs["install"] == True:
        ...
        if kwargs["full"] == True:
            execute_sql_command(
                ...
                username=settings.username,
                password=settings.password,
                sql="""UPDATE rc.surface_admin SET updated = now();""",
            )
...
def get_parser():
    ...
    parser.add_argument(
        "-install",
        default=False,
        help="Install RasterCaster into database",
        dest="install",
        action="store_true",
    )
    parser.add_argument(
        "-cast",
        default=False,
        help="Cast raster (export from database to tif)",
        dest="cast",
        action="store_true",
    )
    parser.add_argument(
        "-full",
        default=False,
        help="Regenerate partial rasters for all surfaces before casting",
        dest="full",
        action="store_true",
    )
    parser.add_argument(
        "-aux",
        default=False,
        help="Generate auxiliary lines for all surfaces. Existing autogenerated lines are overwritten.",
        dest="aux",
        action="store_true",
    )
